...
In this section we will consider 4(1)-tree comparison.
This is the simplest tree comparison which does not follow from Alexandrov comparison.
...
If a Riemannian manifold $M$ satisfies the $4(1)$-tree comparison then 
\begin{enumerate}[(a)]
...
We will  show that for some choice of $\delta,\eps$ and $\zeta$ the array $p,x,x',y,y',q',z$ does not satisfy the $T$-tree comparison with the labeling as on the diagram.
...
For $\delta>0$, define $x'=x'(\delta)$ and $y'=y'(\delta)$ as in~\textit{(\ref{T=>CTIL:CTIL})}.
