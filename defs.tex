...
\end{center}
\unhide
The following theorem staates that these two comparisons also follow form  Alexandrov's comparison.
...
The next bipolar tree comparison is for the tree 4(1).
...
Note that by Reshetnyak majorization theorem,
...
The comparison for the second graph evidently implies that the space is nonnegatively curved,
...
For example, the $(2n+2)$-comparison (which holds in $\CAT[0]$ length spaces, see \cite{AKP}) can be considered as a $(\pm)$-colored graph comparison for the following graph, where $(-)$-edges are marked by solid lines and $(+)$-edges by dashed lines.
