...
Every vertex that is not the root and not a leaf corresponds to a pair of brackets in this notation.
...
these manifolds will be defined and discussed in Section~\ref{sec:cost-convex}.
(Using the terminology of \cite{MTW+CTIL}, cost-convexity=CTIL+MTW.)
...
In this proof, we describe 4(1)-tree comparison using convexity of certain function on the tangent space (see Proposition~\ref{prop:convexity});
we also give an analogous characterization of cost-convex manifolds (see Proposition~\ref{prop:convexity-MTW}).

The proof shows that the spherical 4(1)-tree comparison implies the strict cost-convexity, which in turns implies TCP; see \cite{FRV-Nec+Suf}.
...
\begin{thm}{Exercise}
Let $W\to X$ be a submetry.
Assume $W$ satisfies all tree comparisons.
...
(In general, it is not easy to describe tree comparisons using a system of inequalities.)
...
Note that if a graph $G$ has two induced subgraphs that isomorphic to each of these two graphs, then the corresponding graph comparison implies that the curvature vanish in the sense of Alexandrov.
...
The motivating examples to consider the tree comparison were quotients of Hilbert spac (see Theorem~\ref{thm:hilbert-quotient}).
