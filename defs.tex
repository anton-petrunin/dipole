...
The 4(1)-tree comparison turns out to be related to the  so called \emph{continuity of transport property}, briefly CTP.
...
It is straightforward to check that the spherical 4(1)-tree comparison implies the strict cost-convexity, which in turns implies CTP; see ???.
...
\begin{tikzpicture}[scale=1,
  ...
  \node[main node] (4) at (3/5,-1/3){};
  \node[main node] (5) at (-3/5,-1/3){};
  \node[main node] (6) at (0,+2/3) {};
...
\parbf{Digression.}
Even for a simple monopolar tree as one the diagram it is not easy to describe the corresponding tree comparison using a system of inequalities.
