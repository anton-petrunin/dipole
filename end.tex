...
let us mention authors results in \cite{lebedeva-petrunin} and \cite{petrunin}
and the Markov type inequality proved by Shin-ichi Ohta in \cite{ohta};
see also the survey by Assaf Naor \cite{naor} on the Ribe program.
...
The Alexandrov spaces which satisfy 4(1) seem to have properties which remind the quotients of Riemannian manifolds by isometric group actions. 
...
\parbf{On MTW.}
Let us name the condition (\textit{\ref{thm:convexity:MTW}}) in Theorem~\ref{thm:convexity} by MTW-plus.
As we mentioned, MTW-plus is stronger than MTW, but we fail to show that it is strictly stronger.
In other words, we do not know an answer to the following question:

\begin{thm}{Question}
Is there a Riemannian manifold which is MTW, but not MTW-plus? 
\end{thm}
...
The question might be easier for MTW-plus.

\begin{thm}{Question}
Is it true that MTW (or MTW-plus) implies CTIL?
\end{thm}

\begin{thm}{Question}
Is it true that CTIL+MTW (or CTIL+MTW-plus) on a compact Riemannian manifold implies TCP?
\end{thm}

By Theorem~\ref{thm:convexity}, CTIL+MTW-plus is equivalent to 4(1)-tree comparison. 
Therefore the MTW-plus-vesion of the last question can be reformulated the following way:
